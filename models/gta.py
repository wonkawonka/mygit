...
class GraphTemporalEmbedding(torch.nn.Module):
    def __init__(self, num_nodes, seq_len, num_levels, kernel_size=3, dropout=0.02, device=torch.device('cuda' if torch.cuda.is_available() else 'cpu')):
        ...
        self.gc_modules = AdaGCNConv(num_nodes, seq_len, seq_len)
    ...
    def forward(self, x):
        ...
        x = self.gc_modules(x.transpose(0, 1), self.edge_index).transpose(0, 1) # >> (bsz, num_nodes, seq_len)
        # output = x
